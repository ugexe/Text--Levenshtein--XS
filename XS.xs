    ...
    for (i=0; i < lenSource; i++) {
        ...
        for (j = 0; j < lenTarget; j++) {
            ...
            if( v1[0] == j ) {
                ...
                else if( j < lenSource && mdx < (v1[v1[0]] + diff - MIN(lenSource,lenTarget) - j) ) {
                    warn("i:%d j:%d mdx:%d lenSource:%d lenTarget:%d, v1[0]:%d v1[v1[0]]:%d diff:%d final:%d", i,j,mdx,lenSource,lenTarget,v1[0],v1[v1[0]],diff,(v1[v1[0]] + diff - MIN(lenSource,lenTarget) - j)  );
                    warn("FIRST BLOCK");
